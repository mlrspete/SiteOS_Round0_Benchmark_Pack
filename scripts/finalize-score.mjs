...
if (summary.validEvaluation !== true) throw new Error(`${model.slug} has an invalid evaluator run; rerun evaluation before manual scoring.`)
if (!['completed', 'step-limited'].includes(record.status)) throw new Error(`${model.slug} has non-scorable run status ${record.status}.`)
